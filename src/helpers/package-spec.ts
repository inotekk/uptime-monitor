 ...
 */
export const getPackageName = (packageSpec: string): string => {
  const scopedPackage = packageSpec.match(/^(@[^/]+\/[^@]+)(?:@.+)?$/);
  if (scopedPackage) return scopedPackage[1];

  const unscopedPackage = packageSpec.match(/^([^@:/\s]+)(?:@.+)?$/);
  if (unscopedPackage) return unscopedPackage[1];

  throw new Error(
    `Unable to determine package name from "${packageSpec}". Use a named npm specification.`,
  );
};
